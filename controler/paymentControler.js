...
class paymentController {
    ...
    payment_create = async (req, res) => {
        ...
        try {
            const { data } = await axios.post(process.env.bkash_create_payment_url, {
                mode: '0011',
                payerReference: " ",
                callbackURL: 'https://localhost:3443/api/bkash/payment/callback',
                amount: amount,
                currency: "BDT",
                intent: 'sale',
    ...
    call_back = async (req, res) => {
        ...
        if (status === 'cancel' || status === 'failure') {
            ...
            return res.redirect(`http://localhost:3001/aboutus`)
        }
        if (status === 'success') {
            try {
                ...
                if (data && data.statusCode === '0000') {
                    ...
                    return res.redirect(`http://localhost:3001/menu`)
                }
            } catch (error) {
                console.log(error)
                return res.redirect(`http://localhost:3001/contactus`)
            }
        }
    }
